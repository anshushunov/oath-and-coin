...
`ADR-012` вводит названное исключение из `AGENTS.md` §6 («данные живут в `content/`») со сроком до Task 19. Оно **второе** — первым был `Theme` UI-kit, — а строка §6 всё ещё называет `Theme` единственным. Приведение §6 в соответствие остаётся открытым пунктом и решением владельца, а не исполнителя: он записан в самой `ADR-012`. Третье исключение по §12 п. 3 переписывает правило целиком.
