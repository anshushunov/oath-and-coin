...
| Task 17 — production Electron host | agent | 2026-08-20 | 2026-08-20 | `pnpm package:desktop`; `pnpm test:desktop` | `artifacts/electron-spike/gate-report.json` — расширен полями `userDataPath`, `rendererAssetHashes`, `hostEntryHashes`, `packagedManifest`, `declaredEntryPoints`, `saveRoundTrip`, `strayFilesInRepository`; §18 этого файла | 11 проверок гейта зелёные на **пересобранной** упакованной сборке (было 6); упакованные `main.cjs` и `preload.cjs` читаются из `app.asar` самим работающим процессом и сверяются с `apps/desktop/dist` — именно это ловит устаревший пакет, на котором Task 16.6 осталась зелёной; круг сохранения через `contextBridge` воспроизводит все 256 значений байта, каталог данных — `AppData\Roaming\Oath and Coin`, файлов сохранений в дереве исходников ноль; 307,9 МиБ в 46 файлах установлено и 410,0 МиБ RSS против бюджета 500 МБ; 6 мутантов красят, зелёных нет | принят; `exports` из манифеста хоста убран — упакованный `package.json` рекламировал `./src/contract.ts`, которого в asar нет (замерено внутри asar); побайтовая воспроизводимость сборки electron-builder **не утверждается**; Steam-часть не выполняется (`ADR-011`) |
...
### 18.1. Гейт вырос с 6 проверок до 11
...
| **круг сохранения и чистота дерева** | 256 значений байта туда и обратно; обход рабочего дерева | страница + файловая система |
| **упакованный renderer — это наш бандл** | SHA-256 26 файлов | `resources/web` против `apps/web/dist` |
...
`packagedManifest`, `saveRoundTrip`, `strayFilesInRepository`), плюс одним,
которого план не называл: `declaredEntryPoints` — то, что читает проверка
манифеста. Улика, на которую опирается утверждение, лежит в артефакте рядом с
остальными.
...
путях, а не о наличии зависимостей, и это её граница, а не недосмотр.
...
Слот `slot-c` берётся взаймы: прежнее содержимое читается до записи и
возвращается после, что бы ни утверждал вызывающий. Гейт работает с настоящим
каталогом данных настоящего приложения, и тест, который молча уничтожает
сохранение, — это тест, который запускают с опаской.

### 18.5. Хеши: что утверждается и что только измеряется

**Утверждается** тождество того, что производим мы сами: 26 файлов бандла
renderer (`index.html` плюс 25 в `assets/`) и два входа хоста. Списки
отсортированы по относительному POSIX-пути кодовыми точками, а не
...
Шесть, каждый после коммита проверяемого состояния (`AGENTS.md` §8), каждый
откатан. Зелёных нет.
...
| 6 | байт дописан в `apps/web/dist/index.html` после упаковки | тождество renderer: `index.html` `a549ddbf…` против `5fdbd58f…` | остальные 25 файлов бандла |
...
|---|---|---|
| Проверки упакованного хоста | 11 в 1 файле | `pnpm test:desktop` |
| Установленный размер | 322 814 506 байт (307,9 МиБ) в 46 файлах | тот же прогон, `gate-report.json` |
| RSS | 429 879 296 байт (410,0 МиБ) при бюджете 500 МБ; между прогонами 409–411 МиБ | тот же прогон |
| Процессы | 4: `Browser`, `GPU`, `Utility`, `Tab` | тот же прогон |
| Тесты нового стека | 1143 в 70 файлах | `pnpm test` |
| Parity | 54/54 | `pnpm scenario:parity` |
| Границы | 205 модулей, 677 зависимостей, 0 нарушений | `pnpm lint:deps` |
| Время упаковки | ~58 с | `time pnpm package:desktop` |

Число зависимостей выросло с 676 (§17.1) на одну: гейт теперь статически
импортирует `node:crypto`.
