...
| Task 4 — packaged Electron gate (**stop gate**) | agent | 2026-08-16 | 2026-08-17 | `pnpm package:desktop`; `pnpm test:desktop` | `artifacts/electron-spike/gate-report.json`; §5.2 | 5/5 проверок гейта зелёные на packaged-сборке; 304,2 МиБ установлено и 328,8 МиБ RSS локально, 308,4 МиБ и 254,5 МиБ на CI; 4 мутанта красят | сужен `ADR-011`: Steam изъят, порог размера снят, бюджет RSS сохранён |
| Task 5 — архитектурные границы и dual-stack CI | agent | 2026-08-16 | 2026-08-17 | `pnpm lint:deps`; `pnpm test` | `.dependency-cruiser.cjs`; `.github/workflows/typescript.yml`; §5.3 | 20 модулей, 24 зависимости, 0 нарушений; оба стека зелёные в одном пуше (531 тест .NET + 19 TypeScript); ветка-мутант красит новую стадию | принят |
...
| Task 17 — production Electron host | | | | `pnpm package:desktop`; `pnpm test:desktop` | packaged report, SHA-256, RSS | | пакет ≤ 300 МБ, RSS ≤ 500 МБ |
...
> Записано на выходе из сегмента 1 и здесь оставлено как есть. Что из этого сбылось, а что отменено решением владельца, — в §5.6. Коротко: Steam из миграции изъят (`ADR-011`), стоп-гейт Task 4 пройден 2026-08-17, то есть за две недели до даты обязательного ревью.
...
| Проверки workspace | 12 зелёных | `pnpm test` |
...
Пять проверок, все на запущенном packaged-приложении, ни одна не читает настройки обратно из кода, который их выставил:
...
|---|---:|---:|
| Установленный пакет | 319 026 679 байт (304,2 МиБ), 24 файла | 323 406 869 байт (308,4 МиБ), 24 файла |
| RSS всех процессов | 344 809 472 байт (328,8 МиБ) | 266 862 592 байт (254,5 МиБ) |
...
Разница локального и CI-замера в 4,4 МиБ не расследована и на выводы не влияет: обе величины сняты одной командой, обе далеко от единственного оставшегося бюджета.

### 5.3. Границы и dual-stack CI

| Факт | Значение | Команда |
|---|---|---|
| Модулей под проверкой границ | 20 модулей, 24 зависимости, 0 нарушений | `pnpm lint:deps` |
...
Семнадцать мутантов, каждый поставлен после коммита проверяемого состояния (`AGENTS.md` §8) и откачен `git checkout --`.
...
### 5.6. Что изменилось в решениях за сегмент
...
| HEAD на момент записки | `1e7096e` (Task 5); следом идёт коммит самой записки |
...
| Тесты нового стека | 12 workspace + 2 браузерных + 5 desktop = 19 пройдено (CI run `31972896818`) |

Коммиты сегмента: `06550e5` (workspace) → `ef7ccd1` (LF) → `f687261` (`ADR-011`) → `ebc3e95` (Electron-гейт) → `1e7096e` (границы и CI).
...
4. `vitest.config.ts` трогать не нужно: `include` покрывает `{apps,packages,tests,tools}/**/*.test.ts` шаблоном, а не списком.

### 6.3. Опровергнуто замером

1. **300 МБ на установленный пакет недостижимы.** Пустое Electron-приложение — 304,2 МиБ с уже урезанными локалями. Порог снят `ADR-011`, замер остался. Не возвращать число, не измерив заново.
