...
| Факт | Значение | Команда |
|---|---|---|
| Проверок по workspace | 1132 в 70 файлах (было 847 в 48 на рубеже сегмента 4) | `pnpm test` |
...
| `packages/application` | 139 в 7 файлах | `pnpm --filter @oath-and-coin/application test` |
...
| Голова ветки | предыдущий коммит — `859f69c`; актуальную даёт команда, потому что **эта строка не может назвать коммит, которым она сама вносится** | `git rev-parse --short HEAD` |
| Коммитов от базы `a454fb6` | 33, включая коммит этой правки | `git rev-list --count a454fb6..HEAD` |

Разбивка по типам — 9 `feat`, 13 `fix`, 5 `test`, 6 `docs`
(`git log --format=%s a454fb6..HEAD \| sed 's/(.*//' \| sort \| uniq -c`). Первые
26 — сама Task 16 (`git rev-list --count a454fb6..8d81e92`), последние 7 — закрытие
находок внешнего ревью на шве (`git rev-list --count 8d81e92..HEAD`, §16.10).
...
| Голова | 33 коммита от базы, включая коммит этой правки (`git rev-list --count a454fb6..HEAD`); сам SHA даёт `git rev-parse --short HEAD`, назвать его отсюда нельзя — предыдущий был `859f69c` |
| Дерево | чистое |
| Сделано | **Task 16 целиком** — девять подзадач кода плюс `ADR-006` и эта запись, **плюс закрытие шести находок внешнего ревью на шве** (§16.10). Tasks 17 и 18 не начаты |
| Тестов нового стека | 1132 в 70 файлах (`pnpm test`) |
...
| Внешнее ревью | шов Task 16 **пройден**: шесть находок, три блокирующие, все закрыты (§16.10). Девять внутренних ревью до него (счёт по рабочему журналу вне репозитория — оговорка в §16). Внешнее ревью сегмента целиком — на Task 18 |
