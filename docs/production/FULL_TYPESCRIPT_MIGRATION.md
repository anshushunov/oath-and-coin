...
| Task 17 — production Electron host | agent | 2026-08-20 | 2026-08-20 | `pnpm package:desktop`; `pnpm test:desktop` | `artifacts/electron-spike/gate-report.json` — расширен полями `userDataPath`, `rendererAssetHashes`, `hostEntryHashes`, `packagedManifest`, `declaredEntryPoints`, `saveRoundTrip`, `strayFilesInRepository`, `saveSlotRestored`, `saveScreenStateOnHostRefusal`; §18 этого файла | 12 проверок гейта зелёные на **пересобранной** упакованной сборке (было 6); упакованные `main.cjs` и `preload.cjs` читаются из `app.asar` самим работающим процессом и сверяются с `apps/desktop/dist` — именно это ловит пакет, разошедшийся с последней сборкой, на чём Task 16.6 осталась зелёной; круг сохранения через `contextBridge` воспроизводит все 256 значений байта, файл лежит по пути от `getPath('userData')` в `AppData\Roaming\Oath and Coin`, занятый слот возвращается байт в байт (сверено по SHA-256, копия — на диске, прерванный прогон восстанавливается следующим), файлов сохранений в дереве исходников ноль; отказ обработчика `desktop:save-list` уводит экран слотов в `Error` — путь ошибки, которого нет у браузерной сборки и который до этой задачи не исполнялся ни разу; 307,9 МиБ в 46 файлах установлено и 412,3 МиБ RSS против бюджета 500 МБ; 11 мутантов красят, зелёных нет, и один из них не ловится ничем, кроме нового гейта | принят после внутреннего ревью (1 Critical, 3 Important, 4 Minor, все закрыты); `exports` из манифеста хоста убран — упакованный `package.json` рекламировал `./src/contract.ts`, которого в asar нет (замерено внутри asar); побайтовая воспроизводимость сборки electron-builder **не утверждается**; Steam-часть не выполняется (`ADR-011`) |
...
### 18.1. Гейт вырос с 6 проверок до 12
...
| **круг сохранения, его место и чистота дерева** | 256 значений байта туда и обратно; существование файла по пути от `getPath('userData')`; обход рабочего дерева; восстановление занятого слота по SHA-256 | страница + main-процесс + файловая система |
...
**Где именно оказались байты — отдельное утверждение.** Первая редакция проверяла
только «круг работает» и «в дереве исходников пусто», и ревью показало, что этого
мало: хранилище, укоренённое в `%TEMP%` или в корне диска, проходит обе проверки,
а мутант с `process.cwd()` покраснел лишь потому, что рабочий каталог упакованного
приложения совпал с корнем репозитория — по совпадению, а не по механизму. Теперь
main-процесс сам сообщает путь, выведенный из `getPath('userData')`, и сам
отвечает, лежит ли там файл; мутант с `getPath('temp')` красит именно это
утверждение, оставляя обход дерева пустым (§18.6, мутант 8).
...
предыдущий прогон умер между копированием и возвратом — и копия выигрывает,
`rename`'ом становясь содержимым слота. Иначе восстановление после прерванного
прогона было бы обещанием про человека, который заметит лишний файл и поймёт, что
с ним делать. Проверено вживую: в каталог положена только копия
(`afb70e9f…`), слота нет; после прогона слот есть и содержит ровно её.

**И это единственное место в задаче, где механизм есть, а проверки нет** — сказано
и здесь, и в комментарии у самого кода. Состояние, которое чинит это подбирание,
зелёный прогон устроить не может: оно возникает только после убитого прогона.
Ручной замер выше — измерение, а не механизм; выдавать его за покрытие было бы
ровно тем, что этот сегмент ловил девять раз.
...
Одиннадцать: шесть на первую редакцию и пять на правки по ревью. Каждый — после
коммита проверяемого состояния (`AGENTS.md` §8), каждый откатан. Зелёных нет.
...
**Цена на будущее.** `process.mainModule` помечен в Node как deprecated и в
модуле ESM его нет вовсе. Хост сегодня CommonJS (`main.cjs`, `format: 'cjs'` в
`vite.shared.ts`), так что это работает; в день, когда хост переедет на ESM,
**весь** гейт умрёт разом — четыре его проверки читают файлы этим способом.
Записано здесь, чтобы тот, кто будет переезжать, знал цену заранее.

### 18.8. Числа

| Что | Значение | Команда |
|---|---|---|
| Проверки упакованного хоста | 12 в 1 файле (было 6) | `pnpm test:desktop` |
| Установленный размер | 322 814 506 байт (307,9 МиБ) в 46 файлах | тот же прогон, `gate-report.json` |
| RSS | 432 324 608 байт (412,3 МиБ) при бюджете 500 МБ | тот же прогон |
...
**RSS — живое измерение, и оно двигалось.** По ходу задачи наблюдались 409,3,
410,0, 410,9, 412,3, 413,2, 413,7, 414,5, 415,9, 416,1 и 417,5 МиБ, а ревью на своей
машине видело 412,4. Диапазон
поднялся после того, как гейт стал открывать второй экран: одиннадцатая проверка
уводит страницу на экран слотов, и React рисует его в том же процессе. Запас до
бюджета остаётся большой, но одно число здесь было бы неправдой о свойстве, а не
измерением — поэтому в комментарии у самой проверки стоит диапазон, а точное
значение прогона лежит в `gate-report.json`.
