...
| Task 11 — presentation models | agent | 2026-08-18 | 2026-08-18 | `pnpm --filter @oath-and-coin/presentation test`; `pnpm scenario:parity` | отчёт parity, теперь со сверкой read model; §11.1 | 101 проверка пакета и `tests/locale`, 666 по workspace; 54/54 записи корпуса воспроизведены **вместе с read model, его SHA-256 и состоянием экрана**; 13 мутантов, из них один вернулся зелёным и купил проверку | принят |
...
| Факт | Значение | Команда |
|---|---|---|
| Проверок в пакете | 97 | `pnpm --filter @oath-and-coin/presentation test` |
| Проверок в `tests/locale` | 4 | `pnpm --filter @oath-and-coin/locale-tests test` |
| Проверок по workspace | 666 в 37 файлах | `pnpm test` |
| Записей корпуса | 54/54, теперь включая read model | `pnpm scenario:parity` |
| Границы | 0 нарушений, 123 модуля, 385 зависимостей | `pnpm lint:deps` |
...
Точек вызова две (parity и — в Task 12 — application), обе в гейте.
...
| Тесты нового стека | 666 в 37 файлах |
| Parity | 54/54, теперь включая read model |
| Внешнее ревью | не проводилось (обязательно на Task 15, промежуточное запланировано после Task 12) |

Коммиты: `39367f1` (Task 11) → `d7e23d5` (проверка, купленная зелёным
мутантом) → `6a68847` (журнал).
