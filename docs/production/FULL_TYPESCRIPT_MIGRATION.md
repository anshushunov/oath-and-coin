...
| Факт | Значение | Команда |
|---|---|---|
| Проверок по workspace | 1069 в 67 файлах (было 847 в 48 на рубеже сегмента 4) | `pnpm test` |
| `packages/content` | 216 в 15 файлах | `pnpm --filter @oath-and-coin/content test` |
...
| `packages/application` | 91 в 7 файлах | `pnpm --filter @oath-and-coin/application test` |
| `apps/web` | 155 в 10 файлах | `pnpm --filter @oath-and-coin/web test` |
| `apps/desktop` | 36 в 3 файлах — **пакета с набором тестов до этой задачи не существовало** | `pnpm --filter @oath-and-coin/desktop test` |
| Сценарии и корпус | 149 в 7 файлах | `pnpm test:scenario` |
| Parity | 54/54 воспроизведены, baseline `12565862` | `pnpm scenario:parity` |
| Границы | 200 модулей, 654 зависимости, **0 нарушений** | `pnpm lint:deps` |
...
Коммиты сегмента: `a454fb6..13fa27e`, 24 штуки. Девять из них — `feat`,
одиннадцать — правки по ревью, остальное — тесты и документация.
...
| Голова | `13fa27e`, 24 коммита от базы |
| Дерево | чистое |
| Сделано | **Task 16 целиком** — девять подзадач кода плюс `ADR-006` и эта запись. Tasks 17 и 18 не начаты |
| Тестов нового стека | 1069 в 67 файлах (`pnpm test`) |
...
| Принятые решения | `ADR-006` (стратегия сохранений) и `ADR-012` (тексты интерфейса вне `content/`) — обе `accepted` |
| Внешнее ревью | по швам: девять внутренних ревью пройдено (счёт по рабочему журналу вне репозитория — оговорка в §16), внешнее ревью сегмента — на Task 18 |
...
скрипта `test`; теперь 36 проверок в 3 файлах, атомарная запись через временный
файл и `rename`, сериализация записей по слоту, повтор на транзиентных
`EPERM`/`EBUSY`, IPC-allowlist с Zod на каждый payload и **правило границы в
