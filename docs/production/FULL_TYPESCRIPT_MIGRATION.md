...
| Task 9 — правило решения героя и движок (**stop gate**) | agent | 2026-08-17 | 2026-08-17 | `pnpm --filter @oath-and-coin/simulation test`; `pnpm bench:decision` | `artifacts/decision-benchmark/report.json`; §9.1–§9.3 | 258 проверок симуляции; ворота тратят 0 ординалов; каждый член делится сам и к нулю, а не к −∞; ровно нулевой счёт назван ничьёй; 15 мутантов красят (один вернулся зелёным и купил две проверки); p50 решения 0,0007 мс, p50 команды 0,0096 мс | принят |
| Task 10 — исполнение сценариев и oracle parity | agent | 2026-08-17 | 2026-08-17 | `pnpm test:scenario`; `pnpm scenario:parity` | отчёт parity (`--output`); §9.5–§9.8 | 54/54 записи корпуса воспроизведены **побайтно**; 27 сценариев × 27 checkpoints × 2 seed; все пять кодов `ErrorCodes` достижимы из фикстур, порядок стадий проверен входами, сломанными на двух стадиях сразу; 13 мутантов красят (один вернулся зелёным и купил две проверки) | принят |
...
| `migration/03-core` | 6–10 | обязательно (Task 10) — фактически двумя PR: Tasks 6–8 (PR #13, ревью пройдено, §7.7) и Tasks 9–10 веткой `migration/03-core-decision`; см. §8.2 |
...
| p50/p95 одного решения | 0,0007 / 0,0010 мс на 20 000 замерах | `pnpm bench:decision` |
| p50/p95 одной команды через движок | 0,0096 / 0,0146 мс на 20 000 замерах | `pnpm bench:decision` |
...
гейтом производительности, — имела точку до появления UI-стека. Поэтому
`bench:decision` **не** добавлен в CI: новая стадия pipeline обязана уметь
краснеть (`AGENTS.md` §8), а этой нечем.
...
SHA-256 этого репозитория, другим кодом и по другому пути), а **не** то, на чём
держится сверка. Чего хеш не умеет — сказать, *где* разошлось: он отвечает
«нет» и замолкает. Поэтому сравниваются байты, а при расхождении структуры
обходятся и называется первый путь JSON, который не сошёлся, и первое смещение
в тексте. Мутант, снимающий сравнение байтов и оставляющий только хеш, красит
ровно одну проверку — ту, что требует назвать место. Это и есть точная цена
байтов: обнаружение покупает хеш, локализацию — байты.
...
Порядок важнее самих кодов, поэтому каждая проверка порядка ломает **две**
стадии сразу и утверждает **раннюю**:
...
**Новой CI-стадии не заводится, и это решение, а не забывчивость.** `pnpm test`
уже гоняет и `tests/oracle`, и `tools/scenario-runner`, поэтому отдельная стадия
`scenario:parity` могла бы покраснеть только одновременно со стадией `Test` —
то есть стоила бы прогона, не добавляя ни одного различимого исхода. `AGENTS.md`
...
краснеть только вместе с другой, — тот же расход без того же выигрыша. То же
рассуждение и по той же причине применено к `bench:decision` (§9.1).
...
| Факт | Значение |
|---|---|
| Ветка | `migration/03-core-decision`, от `main` @ `f6276a2` |
| Дерево | чистое |
| Сделано | Tasks 9 и 10 — каждая своим коммитом, мутанты после коммита |
| Сегмент 3 закрыт целиком | Tasks 6–8 слиты PR #13, Tasks 9–10 в этом PR |
| Тесты нового стека | 520 в 31 файле |
...
краснеть) → `75663ef` (код выхода и `matched`) → коммит этой записки.

### 10.2. Что блокирует Task 11

То же правило, что и в трёх предыдущих сегментах: ветка `migration/04-ui`
создаётся от обновлённого `main` после merge этого PR. Плюс своё: Task 11
...
4. **Байты покупают локализацию, а не обнаружение — теперь это замер, а не
   рассуждение.** §8.7 требовала сверять байты, «а хеш — как второй,
   независимый признак», и была права; замер уточняет цену. Мутант, снимающий
   сравнение байтов и оставляющий хеш, красит **одну** проверку — ту, что
   требует назвать место расхождения. Значит формулировка «без байтов
   расхождение не будет замечено» была бы сильнее правды: замечено оно будет,
   не будет названо где.
...
6. **Мутант «таблица команды из `respondedBy`» эквивалентен, пока
...
| Отдельная CI-стадия под parity и под benchmark | Обе могли бы покраснеть только одновременно со стадией `Test`. `AGENTS.md` §8 требует мутанта на новую стадию затем, чтобы в pipeline не попадала стадия, которая не умеет краснеть; стадия, краснеющая только вместе с другой, — тот же расход без выигрыша |
...
- **Двадцать восемь мутантов за сегмент прошли без происшествий** — впервые.
...
- **`error_detail` в модели нет** — там машинно-зависимый путь (§4.5). Не
  добавлять.
- **`ScreenState` — пять значений**, и корпус покрывает все пять:
  `Incomplete` 22 записи, `Normal` 2, `Empty`, `Error`, `Loading` — по одной.
