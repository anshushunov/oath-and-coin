...
| `migration/04-ui` | 11–15 | обязательно (Task 15) — фактически несколькими PR, как сегмент 3: Task 11 (PR #17, ревью пройдено, §11.2), Tasks 12–15 следующими; см. §12.1 |
...
| Факт | Значение | Команда |
|---|---|---|
| Проверок в `packages/content` | 149 в 10 файлах | `pnpm --filter @oath-and-coin/content test` |
| Проверок в `packages/application` | 19 в 2 файлах | `pnpm --filter @oath-and-coin/application test` |
| Проверок по workspace | 709 в 41 файле | `pnpm test` |
| Записей корпуса | 54/54 | `pnpm scenario:parity` |
| Границы | 0 нарушений, 136 модулей, 430 зависимостей | `pnpm lint:deps` |
...
Восемнадцать, и оба класса, про которые §12.7 написала «сегмент покрывал только
первый»: искажение измеряемого и **удаление самой проверки**.
...
| PR | [#17](https://github.com/anshushunov/oath-and-coin/pull/17), открыт на ревью, несёт **только Task 11** |
| Дерево | чистое |
