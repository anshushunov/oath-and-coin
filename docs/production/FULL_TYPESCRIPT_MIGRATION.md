...
| `migration/04-ui` | 11–15 | обязательно (Task 15) — фактически несколькими PR, как сегмент 3: Task 11 (PR #17, ревью пройдено, §11.2), Task 12 (PR #19, ревью пройдено, §11.4), Task 13 (`migration/04-web-screen`, ревью пройдено, §11.6), Tasks 14–15 следующими; см. §12.1 |
...
| Факт | Значение |
|---|---|
| Ветка | `migration/04-web-screen`, создана от `main` @ `a7047f1` |
| Дерево | чистое |
...
свежего `main` под своим именем, а не продолжают эту. Причина та же, что в §2.1,
§12.1 и §13.1: стек PR друг на друге означает, что ревью второго читает дифф
первого.
