...
| `migration/04-ui` | 11–15 | обязательно (Task 15) — фактически несколькими PR, как сегмент 3: Task 11 слита PR #17 (ревью пройдено, §11.2), Tasks 12–15 следующими; см. §12.1 |
...
| Откуда начинать | `main` @ `72d2cd4` — зелёный на обоих стеках, дерево чистое |
| Дерево | чистое |
...
**Сегмент 4 поэтому делится по PR, как сегмент 3.** PR #17 нёс Task 11 и слит;
Tasks 12–15 берут **свежую ветку от `main` @ `72d2cd4`** под своим именем
(`migration/04-ui` удалена вместе с merge). Причина та же, по которой это правило записано в §2.1: стек
