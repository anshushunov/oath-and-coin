...
## Что план A оставил после себя

Раздел обязателен к прочтению до первой строки кода. Он фиксирует контракты в том виде, в каком они реально собраны, а не в том, в каком описаны выше по тексту: план A уточнил несколько решений по ходу, и расхождения здесь уже сведены.

### Изменения контрактов относительно текста ниже

| Что | Как в тексте плана | Как на самом деле |
|---|---|---|
| Действия решения | `Accept = "accept"`, `Decline = "decline"` строками в `ContractDecisionRule` | `Actions.Accept` и `Actions.Decline` — `static readonly ContentId` со значениями `action:accept` и `action:decline` |
| `DecisionResult.SelectedAction` | `string` | `ContentId`; `ConsideredActions` — `ImmutableArray<ContentId>` |
| `TraceFactor.SourceEntity` | `string` | `ContentId`. Герой указывается идентификатором своего определения, а не runtime-идентификатором: определение стабильно между сохранениями |
| `ReasonCodes` | `reason.*` | `hero.decision.*`, остаются строками. Граница: контент-адресуемые сущности получают `ContentId`, закрытый словарь движка — точечные строки |
| Генератор случайности | объект с состоянием | `DeterministicRng.Draw(seed, stream, ordinal)` — чистая функция; `ordinal` типа `ulong` берётся из `GameState.Metadata.NextDecisionOrdinal` |
| `WithEvent` | `WithEvent(DomainEvent, CausalTrace?)` | `WithEvent(DomainEvent, CausalTrace?, ulong drawsConsumed)` — третий параметр обязателен, значения по умолчанию нет |

Значения `action:accept` и `action:decline` **никогда не определяются в контенте**. Валидатор и загрузчик не должны пытаться их разрешать.

### Четыре обязательства, которые компилятор не проверит

1. **`WithEvent` — единственная санкционированная точка перехода.** Выражение `with { }` легально и версию состояния не поднимает; это удерживается комментарием и одним тестом, а не механизмом. Командный слой обязан пропускать через `WithEvent` каждый переход.
2. **`WithEvent` не применяет эффекты события.** Принятие контракта само по себе не меняет ни статус, ни множество ответивших героев — это делает командный слой отдельным `with` перед вызовом `WithEvent`, а не вместо него.
3. **`drawsConsumed` — контракт честности.** Компилятор заставляет передать число, но не правильное число. В сценарии двух героев каждое решение обязано передать `1`; иначе второе объяснение молча повторит первое.
4. **Связка идентификаторов объяснения делается вручную в трёх местах:** `DecisionResult.Trace.TraceId`, `Metadata.NextTraceId` и `CausalTraceId` события. Помощника нет.

### Что теперь работает и на что можно опереться

- **Структурное равенство состояний** починено явными `Equals`/`GetHashCode`; проверено в словарях и множествах. Сравнение состояний в тестах осмысленно.
- **`ContentId` переживает JSON** — конвертер поставляется вместе с типом, включая вариант для ключей словаря. Невалидная строка отвергается с диагностикой, а не превращается в пустое значение.
- **`HeroId` как ключ словаря в JSON не работает** и бросает исключение с внятным текстом. Для плана B это неважно — состояние не сохраняется; при попытке сохранить падение произойдёт сразу, а не позже.
- **Логическое время монотонно:** событие из прошлого отвергается, метаданные подтягиваются вперёд. Два героя в одном тике разрешены — отвергается строго меньшее, не равное.

### Проверено заранее

Ре-ревьюер собрал стенд-ин этого плана внутри сборки симуляции — загрузку определения героя из JSON с полями-целыми и правило решения со счётом по оплате, риску и доверию через `DrawInt32`. Набор остался зелёным, guard границы не дал ложных срабатываний на реалистичном коде загрузки. Ломать контракты плана A не придётся.
