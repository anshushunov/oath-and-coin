...
**Правило действует с 2026-08-26** (`resolution/commitment.ts`, вызывается из `decideHeroResponse`). До него каждое принятие записывалось константой `Committed`: инвариант `commitments.keys() === acceptedBy` действует с той минуты, как поле появилось, значит записывать что-то состояние обязано было уже тогда, а выбрать *что именно* без `DecisionContext` нельзя. Константа была верна для героя без обиды, которого не решила надбавка, и неверна ровно там, где надбавка или обида в игре.

**Порядок трёх шагов — часть правила, а не порядок изложения.** Шаг 1 отвечает *до* контрфакта, а не рядом с ним: у героя с обидой, чей контрфакт всё равно принимает, «обида вперёд» даёт `Resentful`, а «обида как уточнение отказа» — `Committed`. Оба порядка совпадают на герое, чей контрфакт отказывает, поэтому проверять их надо именно на первом случае.
