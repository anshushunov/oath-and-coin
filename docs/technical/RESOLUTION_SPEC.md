...
**`counted` (§4.3) едет со вторым bump'ом, а не со своим третьим — и вот проверка, на которой это держится.** Поле добавляется в `NeedCoverage.contributors`, то есть внутрь `resolution`, а `resolution` в этой сборке **не может быть ничем, кроме `null`**: единственная команда, которая его пишет, — `resolveContract`, и она приходит вместе со вторым bump'ом. Значит пары сейвов «до» и «после», которые это изменение различает, не существует, и правило выше защищать нечего. Утверждение механически проверяемо, а не риторическое: если бы `resolution` где-то был непустым, изменение формы сдвинуло бы канонические слепки — **все 42 остаются побайтово прежними**, и `pnpm test:scenario` это и проверяет. Сдвинулся хоть один — довод неверен и номер должен подняться отдельно.
...
**Код причины у `contract_resolved` — код цели той же ступени** (`ObjectiveTaken` при взятой цели, `ObjectiveLost` при потерянной). §2.1 объявляет девять кодов исхода и ни одного для закрывающего намерения, а поле `reason` у `OutcomeIntent` не допускает пустоты. Разумных ответов три, и выбран третий:
