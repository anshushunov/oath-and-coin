...
export interface NeedCoverage {
  ...
  readonly contributors: readonly {
    ...
    readonly amount: number;    // §4.1 — что человек принёс сам по себе
    readonly counted: number;   // §4.3 — сколько из этого засчиталось после уполовинивания
  }[];
}
...
export interface HeroContribution {
  /** Σ `contributors.amount` этого героя по всем потребностям — что он принёс сам. */
...
**`phase === Settled ⇒ resolution !== null` введено вместе с `resolveContract`, ровно как и было объявлено.** До того `settleContract` рассчитывал заблокированный укомплектованный контракт, не имея никакого разрешения; так делали **семь** поставляемых сценариев, восемью вызовами `settle_contract` (первая редакция этого абзаца говорила «четырнадцать» — число было неверным и исправлено пересчётом по `scenarios/*.commands.json`). Правило пришло с командой, которая единственная может его удовлетворить, вместе с переписыванием этих сценариев и вместе с отказом `settleContract` — `NotResolved`. Отказ и инвариант — две половины одного изменения: без первой команда строила бы запрещённое состояние и роняла конструктор там, где §6.1 требует **отказать**.
...
| `SAVE_SCHEMA_VERSION` | +1 на **поля состояния** (3), +1 на **`counted`** (4), +1 на **варианты событий** (5) |
| `ARTIFACT_VERSION` | так же: поля (5), `counted` (6), события и новая команда (7) |
| `rulesetVersion` | `m1-negotiation/1` → `m1-resolution/1` — **с первым изменением, которое меняет ответ правил** (состояние согласия, 2026-08-26), а не с новой командой |
| `content_version` | дайджест по байтам `content/`; двигается сам, пины и слепки обновляются в том же коммите |

**Почему версия правил двигается раньше остальных — поправка 2026-08-26.** Ранняя редакция отдавала её задаче с командой `resolveContract`, по соседству со вторым bump'ом формата. Это неверно, и `TDD` §7.4 говорит почему: воспроизводимость привязана к тройке `ruleset_version + content_version + seed`, и повторный прогон той же тройки обязан дать тот же результат. Правило состояния согласия меняет **ответ** при неизменной тройке — то же «да» записывается как `fragile` там, где записывалось `committed`. Оставь номер прежним — и кампания, сохранённая под старыми правилами, продолжится под новыми, а проверка `session-controller.ts` не заметит подмены, потому что сравнивать ей будет нечего.

**Отсюда правило, а не разовое решение:** номер правил двигает не новая команда и не новое поле, а любое изменение, после которого прежняя тройка даёт другой ответ. Добавление поля версионируется `SAVE_SCHEMA_VERSION`/`ARTIFACT_VERSION` — это изменение *формата*, и версия правил ему не нужна. Один номер на один набор правил, под которым кто-то мог сохраниться. За этой поправкой ответ поменялся ещё четырежды: `m1-resolution/2` — расчёт стал требовать разрешения (§2.5, задача 8), `m1-resolution/3` — доля заказчика стала функцией ступени (§5.3, задача 9), `m1-resolution/4` — сдвинутый с места кровит (`DEC-017`), `m1-resolution/5` — ход, не меняющий ничего, не берётся: ни статус на его носителя, ни толчок того, кто устоял бы (`COMBAT_SPEC` §5.1). Следующий такой раз берёт `m1-resolution/6`.

**Номер поднимает каждое изменение формы, и считается их столько, сколько их есть.** В этом срезе их три, и они приходят в разное время: поля состояния (задача 3), `counted` в `NeedCoverage.contributors` (задача 7, §4.3), варианты событий и шестая команда (задача 8). Один номер на несколько изменений оставил бы промежуток, где формат изменился, а версия нет: сейв, записанный после первого изменения, читался бы под тем же номером, что и сейв до него. Внутри **одного** изменения поля и события версионируются вместе — добавить вариант события в кодек, не подняв номер, значит то же самое молчание, только внутри одного шага.

**Достижимость формы определяется границей, а не тем, какие пути её сегодня проходят.** Первая редакция изменения `counted` номер не двигала, и довод был такой: `resolution` в этой сборке всегда `null`, потому что единственная команда, которая его пишет, приходит только в задаче 8, — значит различающей пары сейвов не существует. Довод неверен, и ошибка в нём поучительнее вывода. `buildSave` кодирует любой `GameState`, который ему дали, а `createContractState` соберёт разрешённый контракт всякому, кто попросит, — что `snapshot-codec.test.ts` и делает. Проверять надо было **кодек**, а не список команд. Предъявленное доказательство — 42 канонических слепка, оставшихся побайтово прежними — устанавливало только, что ни один из **них** не несёт разрешения, что не то же самое. Найдено внешним ревью; записано потому, что форма ошибки — «проверил вызывающих вместо границы» — представится снова.
...
**Код причины у `contract_resolved` — код цели той же ступени** ([`ADR-015`](../decisions/ADR-015-resolution-draft-shape.md)) (`ObjectiveTaken` при взятой цели, `ObjectiveLost` при потерянной). §2.1 объявляет девять кодов исхода и ни одного для закрывающего намерения, а поле `reason` у `OutcomeIntent` не допускает пустоты. Разумных ответов три, и выбран третий:

- **десятый код** — растит публичный словарь (`OUTCOME_REASON_CODES` попадает в артефакт, в схему сейва и в ключи локализации) ради намерения, которое ничего нового не сообщает;
- **`reason` становится nullable** — меняет тип у всех семи видов намерений ради одного;
- **повторное использование кода цели** — закрывающее намерение говорит ровно то же, что и намерение о цели: сделано дело или нет. Новое в нём — ступень, и её несёт **событие** (§3.4), а не код причины.

Наблюдаемых последствий у выбора нет и это проверяемо: §3.4 не проецирует `reason` ни у одного намерения, а провенанс (§2.1) собирает только намерения, называющие героя или его потребность, — `contract_resolved` не называет ни того, ни другого. Записано здесь, а не оставлено комментарию в коде: у решения были альтернативы, и без записи следующий читатель принял бы его заново.
...
```
counted(h_k, n) = divideTowardZero(contribution(h_k, n), 2^k)
supplied(n)     = Σ counted(h_k, n)
...
**`counted` записывается рядом с `amount`, а не выводится читателем — [`DEC-014`](../decisions/DEC-014-two-numbers-and-who-is-wounded.md) (что показывается) и [`ADR-015`](../decisions/ADR-015-resolution-draft-shape.md) (где хранится), решение владельца 2026-08-27.** Экран разбора показывает у каждого героя **два** числа: что он умеет и сколько из этого засчиталось. Одного мало в обе стороны. Только `amount` — и сумма имён по отряду больше того, что реально получил участок, а четвёртый мечник на экране выглядит полезным; только `counted` — и два одинаковых героя показывают разные числа, а кто из них «первый», решает порядок сортировки, что читается как произвол. Оба вместе делают убывающую отдачу видимой, не выдавая её за свойство человека.

Хранится **по паре (герой, потребность)**, то есть там, где уполовинивание и происходит. Любая сводка — строка героя по всему контракту, итог по участку — тогда получается сложением, а сложение не является вторым утверждением этого правила. Вывод `counted` в слое представления им был бы: он потребовал бы заново применить `2^k` и знать порядок сортировки §4.3.

`HeroContribution.amount` — сумма `amount` этого героя по всем потребностям. Сумма его же `counted` не дублируется полем: она складывается из `coverage`, и второе место, где живёт одно и то же число, — это место, где два числа могут разойтись.
...
| `objective_taken` / `objective_lost` | по ступени: `taken` на «Чисто» и «С ценой», `lost` на «Сорвано» и «Катастрофе» (§5.3) | `0` | — |
...
**«По ступени», а не «по знаку разрыва» — поправка 2026-08-27, внешнее ревью задачи 6.** Первая редакция говорила «по знаку `margin`», и это противоречило §5.3 на целой полосе исходов. Полоса «С ценой» опускается ниже нуля намеренно (§4.6): отряд, не дотянувший одного пункта из ста, дело всё-таки сделал, и §5.3 платит ему полную сумму. По знаку разрыва тот же исход выпускал `objective_lost` — лента говорила «цель потеряна» ровно там, где заказчик платил как за взятую. Решение владельца 2026-08-27: **«С ценой» значит «справились, но заплатили за это»**, поэтому цель следует за ступенью, а знак разрыва остаётся тем, из чего ступень выводится.
...
**Ступень читается из намерений покрытия** — из их вердиктов и из разрыва, который складывается из их же дельт. Если ступень выбрана раньше событий, автономные действия героев не могут причинно породить исход, и трасса объясняет декорацию (`ADR-014`).

**Поправка 2026-08-27 (внешнее ревью задачи 6): читать `objective_taken`/`objective_lost` она не может, и это не упущение, а круг.** Первая редакция требовала, чтобы `gradeFromIntents` смотрела на них; но цель теперь следует за ступенью (§4.4), а ступень — за целью, и вместе это замкнутое определение. Разрывает его порядок, который и был единственным непротиворечивым: **покрытие → намерения покрытия → разрыв → ступень → намерение о цели → последствия → `contract_resolved`**. Требование `ADR-014` при этом выполнено буквально: ступень выведена из того, что сделали герои, а не назначена заранее, — просто «то, что сделали» это вердикты покрытия, а не производное намерение о цели.
...
|---|---|---|---|
| `Wound` | из героев, **отвечающих** (§2.2) за потребность, закрытую хуже всех **среди тех, за которые в отряде отвечает хоть кто-то** (§4.8), — тот, чей личный вклад в неё наибольший: он был на острие | по `HeroId` | `WoundOnThePoint` |
...
**Оговорка «среди тех, за которые отвечает хоть кто-то» — [`DEC-014`](../decisions/DEC-014-two-numbers-and-who-is-wounded.md), решение владельца 2026-08-27.** Первая редакция брала потребность, закрытую хуже всех, без условий, и на достижимом исходе оставляла рану без адресата: потребность, за которую в отряде не отвечает никто, закрыта на ноль и потому хуже всех, а выбирать раненого не из кого — при том что §5.1 объявляет **ровно одну** запись. Два правила противоречили друг другу на реальном входе (две потребности, маленькая незакрытая, отряд без ответственного за неё, ступень «С ценой»). Владелец выбрал: пустая потребность пропускается, рана достаётся тому, кто держал слабейший из **занятых** участков, — «ранят того, кто действительно был на трудном месте», а дыра в составе остаётся дефицитом `coverage_gap` и никого не калечит.

**Остаточный случай, где записей всё-таки меньше объявленного:** отряд, в котором ни один герой не отвечает ни за одну потребность контракта. Тогда занятых участков нет вовсе, и `Wound` не назначается — исход даёт на одну запись меньше. Ни одна команда такого отряда не собирает из поставляемого контента; правило записано, чтобы отсутствие записи было объявленным, а не найденным.
...
| `command-result.ts` | `fromEvents` (§3.3), коды `AlreadyResolved` и `NotResolved` |
| `domain-event.ts` | семь событий исхода, плюс `heroNamedBy` и `isAnswerToAnOffer` |
...
| `validate-game-state.ts` | разбор событий по свойству, а не по перечислению видов |

**`heroNamedBy` и `isAnswerToAnOffer` — не удобство, а исправление формы.** Три места читали событие через список «а вот эти видов героя не называют» (`kind !== 'offer_revised' && kind !== …`). Форма такова, что новый вид событий проходит сквозь неё молча — и семь новых видов пришли одной задачей позже. Оба предиката отвечают на вопрос **положительно**, исчерпывающим `switch` без `default`, поэтому следующий добавленный вид не собирается, пока про него не приняли решение в одном месте. Исчерпывающими обязаны быть **оба**: булева форма `kind === a || kind === b` компилируется в тот же день и молча относит новый вид к «не ответам».

**Свёртка §3.3 названа и отвечает кадрами (`foldOutcome`), и это решение о проверяемости.** §10.3 требует смотреть состояние **после каждого** события, а правило, ради которого свёртка существует, — эффект каждого намерения перед его собственным событием — **не имеет наблюдаемого следствия в итоговом состоянии**: применить все эффекты сначала, а потом провести все события, даёт ту же кампанию, те же события и столь же замороженный результат. Проверка заморозкой это не различает, и внешнее ревью нашло ровно эту дыру. Кадры — наименьший шов, который её закрывает. Из индекса пакета `foldOutcome` **не экспортируется**: снаружи никто не применяет команду вручную, и экспорт был бы приглашением это делать.

**Итог команды замораживается ещё раз, после записи `commandId`.** `withEvent` замораживает состояние, которое возвращает, а спред замороженного объекта даёт свежий **незамороженный**: корень и новый `SortedSet` оставались изменяемыми при сколь угодно глубоко замороженном содержимом. Касалось и `pollCrew`, где та же форма жила с самого начала.
...
**Первые два пункта перестали быть верными для M2, и это записано здесь, а не подразумевается.** Бой описан [`COMBAT_SPEC`](COMBAT_SPEC.md), боевой слой героя — [`DEC-016`](../decisions/DEC-016-hero-combat-layer.md), а роли двух резолверов разведены [`ADR-016`](../decisions/ADR-016-battle-resolver-and-the-forecast.md): контракт, идущий в бой, разрешается боевым резолвером, а арифметика этой спеки остаётся **прогнозом готовности до отправки** и инструментом балансировки. **Ни одно число и ни одно правило §4–§5 при этом не меняется** — меняется то, на каком контракте её выход авторитетен.
