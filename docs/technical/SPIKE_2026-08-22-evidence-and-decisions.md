...
### Что список действительно доходит до модели — замерено на выброшенном коде

**Читай этот раздел иначе, чем остальные.** Всё прочее в записке перепроверяется командой,
которую можно набрать на текущем дереве. Это число — нельзя, и ниже сказано почему.

Зонд целиком, дословно, — он лежал в `tests/oracle/src/spike-probe.ts` и запускался
`node --experimental-strip-types tests/oracle/src/spike-probe.ts` из корня репозитория:
...
for (const scenario of ['screen_normal', 'spike_two']) {
  ...
  if (result.kind !== 'ran') {
    console.log(scenario, '->', result.kind, JSON.stringify(result));
    continue;
  }
  const model = screenFor(result);
  console.log(
    scenario,
    '| steps:', result.outcome.steps.length,
    '| decisions in step 0:', result.outcome.steps[0]?.decisions.length,
    '| screen state:', model.state,
    '| responses:', model.responses.length,
    '| heroes:', model.responses.map((r) => r.heroDefinition).join(','),
    '| read_model_hash:', readModelHash(model)
  );
}
```

Путь не косметика: положенный в корень репозитория, тот же файл падает с
`ERR_MODULE_NOT_FOUND: Cannot find package '@oath-and-coin/application'`. Зонд обязан
лежать внутри пакета воркспейса, отсюда `tests/oracle/src/`.
...
**На текущем дереве этот зонд не запускается, и эквивалента ему в репозитории нет.**
Он опирается на три вещи, которых после Step 5 не существует: поле
`StepOutcome.decisions` (сегодня там `decision` + `heroDefinition`), команду `pollCrew` и
сценарий `spike_two`. CLI сценарного раннера read-model-хеш не печатает вовсе —
`rg "read_model_hash|readModelHash" tools/scenario-runner/src/cli.ts` пуст, — так что
перевывести число из существующих команд нельзя ни обходным путём.

То же равенство наблюдалось второй раз и с другой стороны — из браузера: `report.json`
Замера 2 несёт для `spike_two` тот же `read_model_hash 4695c6e2…`. Это не спасает
воспроизводимость (артефакт получен тем же выброшенным кодом), но означает, что число
пришло из двух разных процессов, а не из одного вызова.

**Число замерено на выброшенном коде. Перепроверить его можно будет после Task 5, и Task 5
обязана это сделать** — см. следующий раздел.

### Обязательство, которое эта записка возлагает на Task 5

Task 5 обязана показать **тестом в репозитории**, а не зондом, что read-model хеш не
зависит от формы: один шаг с N решениями и N шагов по одному решению над одним контрактом
дают одинаковый `readModelHash`. Это ровно то сравнение, которое сделал зонд, и оно
принадлежит `packages/presentation` — это утверждение о фабрике, а не о движке.

Пока такого теста нет, **вердикты «Task 16 подтверждается» и «Task 17 можно сузить» ниже
условны**: оба стоят на том, что список схлопывается в `responses` внутри фабрики и выше
неё не виден, а единственное прямое подтверждение этому — выброшенный зонд. Компиляторная
половина обоих вердиктов (0 правок в `session.ts`, `session-controller.ts` и `apps/web/**`)
перепроверяема и условной не является: её даёт `pnpm typecheck` на изменённом дереве.
...
- **Добавляется обязательство:** тест «read-model хеш не зависит от формы списка» —
  единственное, что вернёт в репозиторий число, замеренное выброшенным зондом (см. раздел
  «Обязательство, которое эта записка возлагает на Task 5»).

**Task 16 — подтверждается, и дешевле, чем в плане; условно.** Контроллер не меняет формы
из-за списка вообще; правка там — это только новые команды, а не новая форма результата.
Утверждение плана `it('returns every decision a crew poll produced')` проверяемо на
контроллере как есть. Перепроверяемая половина — `0` правок в `session.ts` и
`session-controller.ts` по `pnpm typecheck`. Непроверяемая сегодня половина — что список
схлопывается в `responses` без потерь; она держится на выброшенном зонде и станет твёрдой
вместе с тестом Task 5.

**Task 17 — подтверждается, границу можно сузить; условно на том же основании.**
`apps/web/**` не заметил списка решений: 0 правок (перепроверяемо компилятором). Всё, что
там остаётся, — новые поля модели из §5.1 (`offer`, `treasury`, `promiseTerms`,
`settlement`), и это ровно то, что Task 17 и описывает. Списка решений в её объёме нет —
при условии, что фабрика действительно схлопывает список, что и обязана показать Task 5.
...
корпусом, и чинится только решением о том, чем заменить это сравнение.

**Решение владельца (2026-08-23), по этой находке.** Сравнение с `record.canonical_sha256`
снимается в Task 5 как остаток снятого `ADR-013` паритета — круг через сохранение
(`toEqual` над состоянием) остаётся; в Task 20 сила проверки возвращается: ожидаемое
значение берётся из пересобранных `scenarios/*.canonical.json`, то есть снова не из
текущей сборки.
