...
public class GameInvocationTests
{
    ...
    /// side reads back. Width/Height are the one field pair BuildArgv folds
    /// into the engine's own <c>--resolution</c> flag ahead of the
    /// separator (see <see cref="GameArguments.Parse"/>'s remarks) rather
    /// than repeating after it — which is why this uses the default
    /// resolution <see cref="SmokeArguments"/> already carries.
    /// </summary>
    [Fact]
